...
function plot_schrodinger_hermite_timestep(pb::GreedyDiscretization{T}, ::Type{N}, nb_timesteps, G0::AbstractWavePacket{D}, V::AbstractWavePacket{D}; plot_result=true) where{T, D, N}
    ...
    G_list, res_list = schrodinger_greedy_hermite_timestep(pb, N, nb_timesteps, G0, apply_op; progressbar=true)
...
G0 = G0 / norm_L2(G0)
pb = GreedyDiscretization(0., 5., 50, 10, 100, false)
V = Gaussian(1.5, 1.0, -2.0) + Gaussian(1.0, 1.0, 2.0)
plot_schrodinger_hermite_timestep(pb, Tuple{8}, 10, G0, V)